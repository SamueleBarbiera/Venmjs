...
import { dirOfChoice,  readFileContent } from '../../preload/helpers'
...
export default async (deps, { dev }) => {
    await showBanner('VENM', 'Adding dependencies to your project 🧬', 'blue', 'white')
    //checkIfConfigFileExists()

    // Global reference to the directory of choice
    let templateDir = 'client'

    // Get to know whether the deps are to be installed for client/startr directory
    if (fs.existsSync('./startr')) {
    ...
    if (!deps.length && (templateDir === 'startr' || (templateDir === 'client' && template !== 'Nuxt.js'))) {
        logger.warn(' Please specify the dependencies to install'), process.exit(1)
    }
    ...
    if (deps.length) {
        await exec(`npm install ${installFlag} ${deps.join(' ')}`, `Installing ${deps.join(', ')}`, 'Dependencies were successfully installed', {
            cwd: templateDir,
        })
    }
    ...
    if (template === 'Nuxt.js' && !deps.length) {
        // Holds reference to the project specific config (.venmrc)
        //const projectConfig = fetchProjectConfig()

        // Nuxt.js modules that are already installed and configured (.venmrc)
        const { modules: configuredModules, renderingMode } = projectConfig

        // Supported Nuxt.js modules
        let nuxtModules = ['vuetify', 'pwa', 'axios', 'content', 'apollo', 'oauth', 'toast', 'bulma', 'tailwindcss', 'storybook', 'markdownit']
        if (renderingMode === 'spa') {
            // nuxt-oauth requires startr rendered app
        ...
        const availableBuildModules = ['pwa', 'vuetify', 'storybook', 'tailwindcss']

        // Supported Nuxt.js modules
        const availableModules = nuxtModules.filter((module) => !availableBuildModules.includes(module))

        // Nuxt.js modules that are available for installation
        const nuxtDeps = [].concat(nuxtModules, nuxtAddons).filter((dep) => !configuredModules.includes(dep))
        ...
        const modules = installCandidate.filter((dep) => availableModules.includes(dep))
        ...
        if (modules.length) {
            await exec(`npm install --save ${modulesWithPrefix.join(' ')}`, `Installing Nuxt.js modules`, 'Successfully installed the opted Nuxt.js modules', {
                cwd: templateDir,
            })
        }

        // Nuxt.js modules to be installed as a devDependency
        const buildModules = installCandidate.filter((dep) => availableBuildModules.includes(dep))

        // If the user opted for atleast a Nuxt.js buildModule
        if (buildModules.length) {
            // Add @nuxtjs prefix
            const buildModulesWithPrefix = buildModules.map((buildModule) => `@nuxtjs/${buildModule}`)

            // Install buildModules as a devDep
            await exec(`npm install --save-dev ${buildModulesWithPrefix.join(' ')}`, `Installing Nuxt.js buildModules`, 'Successfully installed the opted Nuxt.js buildModules', {
                cwd: templateDir,
            })
        }
        ...
        const buildModulesIdx = nuxtConfig.findIndex((line) => line.includes('buildModules:')) + 2

        let modulesIdx = nuxtConfig.findIndex((line) => line.includes('modules:')) + 2
        ...
        const configureNuxtVuexStore = () => {
            const vuexNuxtStoreTemplate = ['export const state = () => ({', '  counter: 0,', '})', '', 'export const mutations = {', '  increment (state) {', '\tstate.counter++', '  },', '}']
        ...
        if (buildModules.includes('vuetify')) {
            const vuetifyConfig = [`${' '.repeat(2)}vuetify: {`, `${' '.repeat(4)}/* module options */`, `${' '.repeat(2)}},`]
            nuxtConfig.splice(buildModulesIdx, 0, `${' '.repeat(4)}'@nuxtjs/vuetify',`)

            // Add 1 so that the content gets inserted after the buildModules array
            const buildModulesEndIdx = nuxtConfig.indexOf(`${' '.repeat(2)}],`, buildModulesIdx) + 1

            // Add @nuxtjs/vuetify config beneath the buildModules array
            vuetifyConfig.forEach((config, idx) => nuxtConfig.splice(buildModulesEndIdx + idx, 0, config))
        }
        ...
        if (modules.includes('axios')) {
            const axiosConfig = [`${' '.repeat(2)}axios: {`, `${' '.repeat(4)}// proxyHeaders: false`, `${' '.repeat(2)}},`]
            nuxtConfig.splice(modulesIdx, 0, `${' '.repeat(4)}'@nuxtjs/axios',`)

            // Add 1 so that the content gets inserted after the modules array
            const modulesEndIdx = nuxtConfig.indexOf(`${' '.repeat(2)}],`, modulesIdx) + 1

            // Add @nuxtjs/axios config beneath the modules array
            axiosConfig.forEach((config, idx) => nuxtConfig.splice(modulesEndIdx + idx, 0, config))
        }
        ...
        if (modules.includes('content')) {
            const contentConfig = [`${' '.repeat(2)}content: {`, `${' '.repeat(4)} //Options`, `${' '.repeat(2)}},`]
            nuxtConfig.splice(modulesIdx, 0, `${' '.repeat(4)}'@nuxt/content',`)

            const modulesEndIdx = nuxtConfig.indexOf(`${' '.repeat(2)}],`, modulesIdx) + 1

            // Add @nuxt/content config beneath the modules array
            contentConfig.forEach((config, idx) => nuxtConfig.splice(modulesEndIdx + idx, 0, config))
        }

        // Configure @nuxtjs/apollo module
        if (modules.includes('apollo')) {
            const apolloConfig = [`${' '.repeat(2)}apollo: {`, `${' '.repeat(4)}clientConfigs: {`, `${' '.repeat(6)}default: {`, `${' '.repeat(8)}httpEndpoint: 'http://localhost:4000',`, `${' '.repeat(6)}}`, `${' '.repeat(4)}}`, `${' '.repeat(2)}},`]
            nuxtConfig.splice(modulesIdx, 0, `${' '.repeat(4)}'@nuxtjs/apollo',`)

            const modulesEndIdx = nuxtConfig.indexOf(`${' '.repeat(2)}],`, modulesIdx) + 1

            // Add @nuxtjs/apollo config beneath the modules array
            apolloConfig.forEach((config, idx) => nuxtConfig.splice(modulesEndIdx + idx, 0, config))
        }
        ...
        if (modules.includes('oauth')) {
            const oAuthConfig = [
                ...
                `${' '.repeat(4)}oauthClientID: process.env.OAUTH_CLIENT_ID || 'OAUTH_CLIENT_ID',`,
                `${' '.repeat(4)}oauthClientSecret: process.env.OAUTH_CLIENT_SECRET || 'OAUTH_CLIENT_SECRET',`,
            ...
            oAuthConfig.forEach((config, idx) => nuxtConfig.splice(modulesEndIdx + idx, 0, config))
        ...
        if (modules.includes('toast')) {
            const toastConfig = [`${' '.repeat(2)}toast: {`, `${' '.repeat(4)}position: 'top-center',`, `${' '.repeat(4)}register: [ // Register custom toasts`, `${' '.repeat(4)}],`, `${' '.repeat(2)}},`]
            nuxtConfig.splice(modulesIdx, 0, `${' '.repeat(4)}'@nuxtjs/toast',`)

            const modulesEndIdx = nuxtConfig.indexOf(`${' '.repeat(2)}],`, modulesIdx) + 1

            // Add @nuxtjs/toast config beneath the modules array
            toastConfig.forEach((config, idx) => nuxtConfig.splice(modulesEndIdx + idx, 0, config))
        }

        // Configure @nuxtjs/tailwindcss buildModule
        if (buildModules.includes('tailwindcss')) {
            nuxtConfig.splice(buildModulesIdx, 0, `${' '.repeat(4)}'@nuxtjs/tailwindcss',`)
        }
        ...
        if (modules.includes('bulma')) {
            const postCssConfig = [`${' '.repeat(4)}postcss: {`, `${' '.repeat(6)}preset: {`, `${' '.repeat(8)}features: {`, `${' '.repeat(10)}customProperties: false`, `${' '.repeat(8)}}`, `${' '.repeat(6)}}`, `${' '.repeat(4)}},`]
            nuxtConfig.splice(modulesIdx, 0, `${' '.repeat(4)}'@nuxtjs/bulma',`)

            // Add 4 so that the content gets inserted at the right position
            const buildConfigIdx = nuxtConfig.findIndex((line) => line.includes('build:')) + 4
            postCssConfig.forEach((config, idx) => nuxtConfig.splice(buildConfigIdx + idx, 0, config))
        }

        // Configure @nuxtjs/storybook buildModule
        if (buildModules.includes('storybook')) {
            const storybookConfig = [`${' '.repeat(2)}storybook: {`, `${' '.repeat(4)} // Options`, `${' '.repeat(2)}},`]

            const modulesEndIdx = nuxtConfig.indexOf(`${' '.repeat(2)}],`, modulesIdx) + 1

            // Add @nuxtjs/storybook config beneath the modules array
            storybookConfig.forEach((config, idx) => nuxtConfig.splice(modulesEndIdx + idx, 0, config))
        ...
        if (modules.includes('markdownit')) {
            const markdownitConfig = [`${' '.repeat(2)}markdownit: {`, `${' '.repeat(4)}preset: 'default',`, `${' '.repeat(4)}linkify: true,`, `${' '.repeat(4)}breaks: true,`, `${' '.repeat(4)}use: [`, `${' '.repeat(6)}'markdown-it-div',`, `${' '.repeat(6)}'markdown-it-attrs',`, `${' '.repeat(4)}],`, `${' '.repeat(2)}},`]
            nuxtConfig.splice(modulesIdx, 0, `${' '.repeat(4)}'@nuxtjs/markdownit',`)

            const modulesEndIdx = nuxtConfig.indexOf(`${' '.repeat(2)}],`, modulesIdx) + 1

            // Add @nuxtjs/toast config beneath the modules array
            markdownitConfig.forEach((config, idx) => nuxtConfig.splice(modulesEndIdx + idx, 0, config))
        }
        ...
        if (!isConfigured[templateDir]) {
            // Additional dependencies were installed before invoking start
            await exec('npm install', 'Installing dependencies in the background. Hold on...', 'Dependencies were successfully installed', {
                cwd: templateDir,
            })

            // Update .venmrc
            projectConfig.isConfigured[templateDir] = true
            fs.writeFileSync('.venmrc', JSON.stringify(projectConfig, null, 2))
        }
        ...
        if (deps.includes('vuex')) {
            ...
            const vuexStoreTemplate = [`import Vue from "vue";`, `import Vuex from "vuex";`, '', `Vue.use(Vuex);`, '', `export default new Vuex.Store({`, `${' '.repeat(2)}state: {},`, '', `${' '.repeat(2)}getters: {},`, '', `${' '.repeat(2)}mutations: {},`, '', `${' '.repeat(2)}actions: {}`, `});`, '']
            ...
            const routerIndex = config.indexOf(config.find((line) => line.trim() === 'router,'))
        ...
        if (deps.includes('vuetify')) {
            ...
            ;[`import Vuetify from "vuetify";`, `import "vuetify/dist/vuetify.min.css";`].forEach((item, i) => config.splice(i + 1, 0, item))

            // Fetch the index after which the respective config should come up
            const preIndex = config.indexOf(config.find((line) => line.includes('Vue.config.productionTip')))

            // Inserting the respective Vuetify config
            ;['Vue.use(Vuetify);', ''].forEach((item, i) => config.splice(preIndex + i, 0, item))
